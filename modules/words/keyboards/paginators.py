...
async def create_subject_paginator(table_name: str):
    ...
    def group_subject_paginator(page: int = 0):
        ...
        if count_groups > 10:
            ...
            builder.adjust(1)
            builder.row(
                InlineKeyboardButton(text="⬅️", callback_data=GroupSubjectPagination(action="prev", page=page, db_table=table_name).pack()),
                InlineKeyboardButton(text="➡️", callback_data=GroupSubjectPagination(action="next", page=page, db_table=table_name).pack()),
